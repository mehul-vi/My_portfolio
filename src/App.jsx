...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0C] text-[#F4F4F5] font-sans antialiased selection:bg-white/10 overflow-x-hidden relative">
      ...
      <header className="sticky top-0 z-50 w-full border-b border-white/[0.05] bg-[#0A0A0C]/80 backdrop-blur-md px-6">
        <div className="max-w-4xl mx-auto h-24 flex items-center justify-between">
          ...
          <motion.a
            href="/Mehul_Kumar_Sahu_Resume.pdf"
            download
          ...
          <div className="max-w-md mx-auto p-1.5 border border-zinc-900 bg-zinc-950/80 rounded-2xl flex items-center justify-center gap-3.5 shadow-lg">
            {projects.map((proj, idx) => (
              <button
                ...
                className={`relative aspect-video w-[76px] rounded-lg overflow-hidden border-2 transition-all duration-300 cursor-pointer ${
              ...
              >
                ...
                {/* Number Overlay */}
                <div className={`absolute inset-0 flex items-center justify-center text-[11px] font-mono font-extrabold ${
                  activeProjectIdx === idx ? "bg-fuchsia-950/60 text-fuchsia-300" : "bg-black/60 text-zinc-400"
                }`}>
                  {idx + 1}
                </div>
              </button>
      ...
      {/* Sleek Minimalist Spring-Physics Cursor follower */}
      <div className="hidden md:block">
        ...
        <motion.div
          ...
          animate={{
            ...
            marginLeft: cursorHovered ? -22 : -12,
            marginTop: cursorHovered ? -22 : -12,
            borderColor: cursorHovered ? "rgba(56, 189, 248, 0.6)" : "rgba(255, 255, 255, 0.12)",
            backgroundColor: cursorHovered ? "rgba(56, 189, 248, 0.04)" : "rgba(255, 255, 255, 0.01)",
          }}
          transition={{ type: "spring", stiffness: 350, damping: 25 }}
          className="fixed top-0 left-0 border rounded-full z-50 pointer-events-none shadow-lg shadow-black/10"
        />
        ...
        <motion.div
          style={{ x: cursorX, y: cursorY }}
          animate={{
            scale: cursorHovered ? 1.25 : 1,
          }}
          className="fixed top-0 left-0 w-1.5 h-1.5 -ml-[3px] -mt-[3px] bg-sky-400 rounded-full z-50 pointer-events-none shadow-[0_0_8px_#38bdf8]"
        />
      </div>
